        ...
        {#each products as product, index}
          <tr class:opacity-30={!product.active}>
            <td>
              {#if product.images && product.images.length > 0}
                <a href="/uploads/{product.images[0].name}.{product.images[0].ext}" target="_blank">
                  <img style="width: 100%; max-width: 80px" src="/uploads/{product.images[0].name}_sm.{product.images[0].ext}" alt={product.name} loading="lazy" />
                </a>
              {:else}
            ...
            <td on:click={() => openView(product, index)}>
              ...
              {#if product.brief}
                <span class="text-gray-400 hidden xl:block">{product.brief}</span>
              {/if}
            </td>
            <td>
              {#if product.active}
            ...
            <td on:click={() => openView(product, index)}>
              {costFormat(product.amount)} {currency}
            </td>
            <td class="px-4 py-2">
        ...
        <form on:submit|preventDefault={handleSubmit}>
            ...
            <dl class="-my-3 mx-auto mb-0 mt-2 space-y-4 text-sm">
              ...
              {#each formData.metadata || [] as metadata, index}
                <div class="flex">
                  ...
                  <div class="flex-none cursor-pointer pl-3 pt-3" role="button" tabindex="0" on:click={() => deleteMetadataRecord(index)} on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); deleteMetadataRecord(index); } }}>
              ...
              <div class="flex">
                ...
                <div class="mt-2 flex-none">
                  <button type="button" class="shrink-0 rounded-lg bg-gray-200 p-2 text-sm font-medium text-gray-700" on:click={addMetadataRecord}>
                    Add metadata record
                  </button>
                </div>
              </div>
                ...
                <div class="flex">
                  <div class="grow">
                    <FormInput
                      ...
                      type="text"
                      title=""
                      bind:value={formData.attributes[index]}
                    />
                  </div>
                  <div class="flex-none cursor-pointer pl-3 pt-3" role="button" tabindex="0" on:click={() => deleteAttributeRecord(index)} on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); deleteAttributeRecord(index); } }}>
                    <SvgIcon name="trash" className="h-5 w-5" stroke="currentColor" />
                  </div>
                </div>
              {/each}
              <div class="flex">
                <div class="grow"></div>
                <div class="mt-2 flex-none">
                  <button type="button" class="shrink-0 rounded-lg bg-gray-200 p-2 text-sm font-medium text-gray-700" on:click={addAttributeRecord}>
                    ...
                    {#each productImages as image, index}
                      <div class="relative" style="width: 100%; max-width: 150px">
                        ...
                        <div 
                          ...
                          on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); deleteProductImage(index); } }}
                        >
                          <SvgIcon name="trash" className="h-5 w-5" stroke="currentColor" />
                        </div>
                      </div>
          ...
          <div class="pt-8">
            <div class="flex">
              ...
              {#if drawerMode === 'edit' && fullProductData}
                <div class="mt-2 flex-none">
                  <span on:click={handleDeleteProduct} on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleDeleteProduct(); } }} role="button" tabindex="0" class="cursor-pointer text-red-700">Delete</span>
                </div>
              {/if}
            </div>
          </div>
