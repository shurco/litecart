...
<svelte:component this={Main}>
  ...
  {:else if carts.length === 0}
    <div class="text-center py-8 text-gray-500">No carts found</div>
  {:else}
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {#each carts as cart}
          <tr class:bg-green-50={cart.payment_status === 'paid'}>
            ...
            <td class={cart.payment_status === 'paid' ? 'text-green-600' : cart.payment_status === 'pending' ? 'text-yellow-600' : cart.payment_status === 'failed' ? 'text-red-600' : 'text-gray-600'}>
              {cart.payment_status || '-'}
            </td>
