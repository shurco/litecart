  ...
  $: mainMenuSections = (() => {
    if (currentRoute?.includes('/settings')) {
      return [
        ...
        { name: 'settingsWebhook', path: `${base}/settings/webhook`, meta: { ico: 'webhook', title: 'Webhook events' } },
...
<div class="relative flex h-screen overflow-hidden">
  <div class="flex h-screen w-16 flex-col justify-between border-e border-e-gray-200 bg-white">
        ...
        <ul class="py-4">
          {#each mainMenu as item}
            <li class="pb-2 {item.meta.divider ? 'space-y-1 border-t border-gray-100 pt-2' : ''}">
              <a href={item.path} data-sveltekit-preload-data="hover" class="group relative flex justify-center rounded px-2 py-1.5 {currentRoute === item.path || (item.path === `${base}/settings` && currentRoute?.startsWith(`${base}/settings`)) ? 'text-blue-700 bg-blue-100' : 'text-gray-500 hover:bg-gray-200 hover:text-gray-700'}">
    ...
    <div class="sticky left-0 right-0 bottom-0 border-t border-gray-100 bg-white px-2">
      <ul class="pt-2">
        <li class="pb-2">
         ...
         <li class="pb-2">
           <a href={`${base}/signin`} on:click|preventDefault={signOut} class="block" aria-label="Sign out">
             <div class="group relative flex justify-center rounded px-2 py-1.5 text-gray-500 hover:bg-red-100 hover:text-red-700">
    ...
    <div class="h-screen w-52 flex-col justify-between border-e border-e-gray-200 bg-white px-2">
      ...
      <ul class="space-y-1 mt-1.5">
        {#each mainMenuSections as item}
          <li>
            <a href={item.path} data-sveltekit-preload-data="hover" class="flex items-center gap-2 rounded px-4 py-2 {currentRoute === item.path ? 'text-blue-700 bg-blue-100' : 'text-gray-500 hover:bg-gray-200 hover:text-gray-700'}">
  ...
  <div class="flex flex-grow flex-col overflow-x-hidden overflow-y-auto">
    ...
    <div class="sticky left-0 right-0 bottom-0 h-12 border-t border-t-gray-200 bg-zinc-50 flex">
      ...
      <div 
        ...
        on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goToRelease(); } }}
        class="cursor-pointer flex-none p-4 text-xs {(version.new ? 'border-gray-300 bg-yellow-200 text-gray-500' : 'border-gray-100 text-gray-300')}" 
