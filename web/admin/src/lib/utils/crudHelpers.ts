...
export function createCrudOperations<T extends { id: string }>(baseUrl: string, entityName: string): CrudOperations<T> {
  return {
    load: async () => {
      return loadData<T[]>(baseUrl, `Failed to load ${entityName}`)
    },
    create: async (data: Partial<T>) => {
      return saveData<T, Partial<T>>(baseUrl, data, false, `${entityName} created`, `Failed to create ${entityName}`)
    },
    ...
    delete: async (id: string) => {
      return deleteData(`${baseUrl}/${id}`, `${entityName} deleted`, `Failed to delete ${entityName}`)
    },
