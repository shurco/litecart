...
<div>
  <div class="pb-8">
    <div class="flex items-center">
      ...
      <div>
        {#if product}
          <SvgIcon name={product.active ? 'eye' : 'eye-slash'} className="h-5 w-5 cursor-pointer" on:click={active} stroke="currentColor" />
        {/if}
      </div>
    </div>
  </div>
          ...
          {#each product.images as item}
            <div>
                <a href="/uploads/{item.name}.{item.ext}" target="_blank" aria-label="View full size image">
                  <img style="width: 100%; max-width: 150px" src="/uploads/{item.name}_sm.{item.ext}" alt="{product.name} - {item.name}" loading="lazy" />
              </a>
            </div>
          {/each}
        </DetailList>
