...
<button class="group relative inline-flex items-center overflow-hidden rounded px-8 py-2 text-white cursor-pointer {colorClasses} {icoClasses}" {type} on:click={handleClick} {...$$restProps}>
  {#if ico}
    <SvgIcon name={ico} stroke="currentColor" className="h-4 w-4 absolute -start-full transition-all group-hover:start-4" />
  {/if}
